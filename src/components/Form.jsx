...
function Form() {
  ...
  const { createCity, isLoading } = useCities(); // Updated function name
  const navigate = useNavigate();
  const [cityName, setCityName] = useState("");
  const [country, setCountry] = useState("");
  const [date, setDate] = useState(null); // Initial value as null
  ...
  useEffect(() => {
    const fetchCityData = async () => {
      if (!lat || !lng) {
        setGeoCodingError("Latitude and longitude are missing.");
        return;
      }
